...
<div class="row">
    <div class="col-lg">
        ...
        {!! Form::open(['action' => ['ActivitiesController@update', $activity->id], 'method' => 'post', 'autocomplete' => 'off', 'enctype' => 'multipart/form-data']) !!}
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    {{ Form::text('name', $activity->name, ['class' => 'form-control', 'placeholder' => 'Give your activity a title']) }}
                </div>
                <div class="form-group">
                    {{ Form::label('tagline', 'Activity tagline') }}
                    {{ Form::text('tagline', $activity->tagline, ['class' => 'form-control', 'placeholder' => 'Describe your activity in three words', 'maxlength' => '50']) }}
                </div>
                <div class="form-group">
                    {{ Form::label('description', 'Activity description') }}
                    {{ Form::textarea('description', $activity->description, ['class' => 'form-control']) }}
                </div>
            </fieldset>
            <fieldset>
                <div class="field-header">
            ...
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    {{ Form::number('price', $activity->price, ['class' => 'form-control']) }}
                </div>
                <div class="form-group">
                    {{ Form::label('category', 'Category') }}
                    <select class="form-control" name="category">
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}"{{ $activity->category_id == $category->id ? "selected" : "" }}>{{ $category->category }}</option>
                    @endforeach
                    </select>
                </div>
                <div class="form-group">
                    {{ Form::label('term', 'Term or session?') }}
                    {{ Form::select('term', ['term' => 'Term', 'session' => 'Session'], $activity->term, ['class' => 'form-control']) }}
                </div>
                ...
                @if($activity->term == "term" && $term->count() > 0)
                <div class="form-group" id="TermStart">
                    <input type="hidden" name="term_id" value="{{ $term[0]->id }}">
                    {{ Form::label('term_start', 'Term start') }}
                    {{ Form::text('term_start', $term[0]->start, ['class' => 'form-control']) }}
                </div>
                <div class="form-group" id="TermEnd">
                    {{ Form::label('term_end', 'Term end') }}
                    {{ Form::text('term_end', $term[0]->end, ['class' => 'form-control']) }}
                </div>
                @else
                <div class="form-group" id="TermStart">
                    {{ Form::label('term_start', 'Term start') }}
                ...
                <div class="form-group">
                    ...
                    {{ Form::number('spots_available', $term[0]->spots_available, ['class' => 'form-control']) }}
                </div>
            </fieldset>
